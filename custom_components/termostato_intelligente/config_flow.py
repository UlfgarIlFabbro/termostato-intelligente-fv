...
def _schema_energia(defaults: dict) -> vol.Schema:
    return vol.Schema({
        ...
        _f(vol.Optional, CONF_FV_MARGIN_W, defaults, DEFAULT_FV_MARGIN_W): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0, max=5000, step=50, unit_of_measurement="W", mode="box")
        ),
        ...
        _f(vol.Optional, CONF_FV_PRIORITY, defaults, DEFAULT_FV_PRIORITY): selector.NumberSelector(
            selector.NumberSelectorConfig(min=1, max=99, step=1, mode="box")
        ),
        _f(vol.Optional, CONF_FV_STAGGER_MIN, defaults, DEFAULT_FV_STAGGER_MIN): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0, max=60, step=1, unit_of_measurement="min", mode="box")
        ),
        # --- Spegnimento automatico da FV ---
        _f(vol.Optional, CONF_FV_SHUTOFF_ENABLED, defaults, DEFAULT_FV_SHUTOFF_ENABLED): selector.BooleanSelector(),
        _f(vol.Optional, CONF_FV_SHUTOFF_DELAY_MIN, defaults, DEFAULT_FV_SHUTOFF_DELAY_MIN): selector.NumberSelector(
            selector.NumberSelectorConfig(min=1, max=60, step=1, unit_of_measurement="min", mode="box")
        ),
        _f(vol.Optional, CONF_FV_SHUTOFF_EXTRA_HOURS, defaults, DEFAULT_FV_SHUTOFF_EXTRA_HOURS): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0, max=4, step=0.5, unit_of_measurement="h", mode="box")
        ),
    })
...
def _schema_soglie_termiche(defaults: dict) -> vol.Schema:
    return vol.Schema({
        _f(vol.Optional, CONF_TARGET_TEMP_DEFAULT, defaults, DEFAULT_TARGET_TEMP): selector.NumberSelector(
            selector.NumberSelectorConfig(min=16, max=30, step=0.5, unit_of_measurement="°C", mode="box")
        ),
        _f(vol.Optional, CONF_EXTREME_OFFSET, defaults, DEFAULT_EXTREME_OFFSET): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0, max=8, step=0.1, unit_of_measurement="°C", mode="box")
        ),
        _f(vol.Optional, CONF_HOT_OFFSET, defaults, DEFAULT_HOT_OFFSET): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0, max=5, step=0.1, unit_of_measurement="°C", mode="box")
        ),
        _f(vol.Optional, CONF_RANGE_OFFSET, defaults, DEFAULT_RANGE_OFFSET): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0, max=5, step=0.1, unit_of_measurement="°C", mode="box")
        ),
        _f(vol.Optional, CONF_BELOW_OFFSET, defaults, DEFAULT_BELOW_OFFSET): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0, max=3, step=0.05, unit_of_measurement="°C", mode="box")
        ),
        _f(vol.Optional, CONF_TURN_ON_OFFSET, defaults, DEFAULT_TURN_ON_OFFSET): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0, max=5, step=0.1, unit_of_measurement="°C", mode="box")
        ),
        _f(vol.Optional, CONF_TEMP_DELTA, defaults, DEFAULT_TEMP_DELTA): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0.5, max=5, step=0.5, unit_of_measurement="°C", mode="box")
        ),
        _f(vol.Optional, CONF_EXTREME_DELTA, defaults, DEFAULT_EXTREME_DELTA): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0.5, max=8, step=0.5, unit_of_measurement="°C", mode="box")
        ),
    })
...
def _schema_soglie_avanzate(defaults: dict) -> vol.Schema:
    return vol.Schema({
        _f(vol.Optional, CONF_CALIBRATION_MAX_OFFSET, defaults, DEFAULT_CALIBRATION_MAX_OFFSET): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0, max=8, step=0.5, unit_of_measurement="°C", mode="box")
        ),
        _f(vol.Optional, CONF_MIN_BELOW_INTERNAL, defaults, DEFAULT_MIN_BELOW_INTERNAL): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0, max=5, step=0.5, unit_of_measurement="°C", mode="box")
        ),
        # --- Modalità notturna ---
        ...
        _f(vol.Optional, CONF_NIGHT_OFFSET, defaults, DEFAULT_NIGHT_OFFSET): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0, max=5, step=0.5, unit_of_measurement="°C", mode="box")
        ),
        _f(vol.Optional, CONF_NIGHT_AC_ENABLED, defaults, DEFAULT_NIGHT_AC_ENABLED): selector.BooleanSelector(),
        _f(vol.Optional, CONF_NIGHT_TURN_ON_OFFSET, defaults, DEFAULT_NIGHT_TURN_ON_OFFSET): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0, max=6, step=0.5, unit_of_measurement="°C", mode="box")
        ),
        ...
        _f(vol.Optional, CONF_NIGHT_SHUTOFF_DELTA, defaults, DEFAULT_NIGHT_SHUTOFF_DELTA): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0, max=3, step=0.1, unit_of_measurement="°C", mode="box")
        ),
        _f(vol.Optional, CONF_NIGHT_SHUTOFF_MIN, defaults, DEFAULT_NIGHT_SHUTOFF_MIN): selector.NumberSelector(
            selector.NumberSelectorConfig(min=5, max=120, step=5, unit_of_measurement="min", mode="box")
        ),
        # --- Presenza e timing ---
        _f(vol.Optional, CONF_PRESENCE_BOOST_ENABLED, defaults, DEFAULT_PRESENCE_BOOST_ENABLED): selector.BooleanSelector(),
        _f(vol.Optional, CONF_PRESENCE_BOOST_MIN, defaults, DEFAULT_PRESENCE_BOOST_MIN): selector.NumberSelector(
            selector.NumberSelectorConfig(min=1, max=60, step=1, unit_of_measurement="min", mode="box")
        ),
        _f(vol.Optional, CONF_PRESENCE_BOOST_OFFSET, defaults, DEFAULT_PRESENCE_BOOST_OFFSET): selector.NumberSelector(
            selector.NumberSelectorConfig(min=0, max=5, step=0.1, unit_of_measurement="°C", mode="box")
        ),
        _f(vol.Optional, CONF_WINDOW_DELAY_MIN, defaults, DEFAULT_WINDOW_DELAY_MIN): selector.NumberSelector(
            selector.NumberSelectorConfig(min=1, max=60, step=1, unit_of_measurement="min", mode="box")
        ),
        _f(vol.Optional, CONF_UPDATE_INTERVAL_MIN, defaults, DEFAULT_UPDATE_INTERVAL_MIN): selector.NumberSelector(
            selector.NumberSelectorConfig(min=1, max=30, step=1, unit_of_measurement="min", mode="box")
        ),
    })
...
def _schema_notifiche(defaults: dict) -> vol.Schema:
    return vol.Schema({
        ...
        _f(vol.Optional, CONF_TTS_MESSAGE_OPEN, defaults, DEFAULT_TTS_MESSAGE_OPEN): selector.TextSelector(
        ...
        _f(vol.Optional, CONF_NOTIFY_MESSAGE, defaults, DEFAULT_NOTIFY_MESSAGE): selector.TextSelector(
            selector.TextSelectorConfig(multiline=True)
        ),
        _f(vol.Optional, CONF_NOTIFY_TEMP_CHANGE_ENABLED, defaults, DEFAULT_NOTIFY_TEMP_CHANGE_ENABLED): selector.BooleanSelector(),
        _f(vol.Optional, CONF_NOTIFY_TEMP_CHANGE_MESSAGE, defaults, DEFAULT_NOTIFY_TEMP_CHANGE_MESSAGE): selector.TextSelector(
            selector.TextSelectorConfig(multiline=True)
        ),
        ...
        _f(vol.Optional, CONF_DOOR_ALERT_MESSAGE, defaults, DEFAULT_DOOR_ALERT_MESSAGE): selector.TextSelector(
            selector.TextSelectorConfig(multiline=True)
        ),
    })
